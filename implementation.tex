...
\begin{figure}\centering\includegraphics[width=\textwidth]{implementation-events}\caption{State and socket events sent and consumed by controls in the application, grouped by library.}\label{fig:implementation-events}\end{figure}
...
This means the presenter has to find these next slides, using the router's information about the navigatable directions, and render them in a designated area. As mentioned before, special attention was paid to mobile stylesheets to make the presenter view usable on mobile devices. This ensures that everything is big enough to be readable and all buttons are clickable.
...
The last group of components connected to the interactive extension covered in this chapter allow speakers to create votings (both during in the preparation of the presentation and on-the-fly as shown in figure \ref{fig:implementation-interactive-voting}) and members of the audience to vote.
